...
| CMP-015 | **Set table row** | S-006 | `Set · Previous · Kg · Reps · ✓`. Completed row tinted green full-width; pending values **pre-filled muted from the routine's prescription**; `Previous` separately shows last session's actual; ✓ is a filled circle in both states. | Rework of `strength_set_row.dart` **plus a template data change** — see CMP-017. **The highest-detail pattern in the whole teardown.** | **Partly built** — plan pre-fill shipped (T-008); `Previous` and the green tint outstanding |
